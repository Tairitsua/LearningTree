...
- 分类：接口可访问性 / 默认实现 / 代理分派
- 来源：`FIPS2022/src/Shared/BuildingBlocksPlatform/SeedWork/OurRepository.cs`
- 现象：`internal` 接口方法即使已在基类实现，继承链或 Castle 一类运行时代理激活时仍可能被判定为未实现；如果接口还带默认实现，运行时还可能直接落到接口默认实现。
- 理解：编译通过不等于代理框架的运行时绑定一定正确，可见性、显式接口实现和代理程序集访问能力都会影响最终分派。
- 实践：需要继承、拦截、代理的接口成员优先保持 `public`；避免把核心扩展点设计成 `internal` 接口方法。
