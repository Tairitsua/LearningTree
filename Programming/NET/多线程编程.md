...
### UnobservedTaskException 的触发时机

- 分类：Task 异常观察 / GC 时机
- 来源：`Monica.Core/Modules/ModuleGlobalExceptionHandler.cs`
- 现象：`TaskScheduler.UnobservedTaskException` 不会在任务刚失败时立刻触发，通常要等故障 `Task` 一直未被观察并且后续被 GC 回收。
- 理解：它更像最终兜底告警，不是主异常链路，也不能用于精确、实时地捕获后台任务失败。
- 实践：`fire-and-forget` 任务也要主动 `await`、封装统一托管，或在任务体内自行记录异常；不要把异常处理职责压给该事件。
