...
### `string` 与 `string?` 的运行时关系

- 分类：引用类型 / 可空引用类型
- 来源：`Monica.AutoModel/Implements/AutoModelSnapshotMemoryProvider.cs`
- 结论：`string` 是引用类型，`string?` 并不是另一个 CLR 类型；运行时仍然是 `System.String`，差异主要体现在可空上下文元数据和编译器流分析。
- 实践：做反射或 `typeof` 判断时，`typeof(string)` 同时覆盖 `string` 和 `string?`；不要把 `string?` 理解成 `Nullable<string>`。
